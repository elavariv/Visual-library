...
//3.2 Create a group for the texts inside the circles
const graphText = canvas.append('g')
    .attr('width', graphWidth)
    .attr('height', graphHeight)
    .attr('transform', `translate(${margin.left},${margin.top})`);

//6.1 Create a scale to map colors
...
const update = (data) => {
    ...
    const bubbles = graph.selectAll('g') .data(data); 
    //data() returns an object that conatins three sub objects - entry, exit, groups & parents

    //6.2 add domain to the scale
    bubbleColor.domain([d3.min(data, d=>d.r), d3.max(data, d=>d.r)])

    //5.1. Handle new element creation for the cicles
    var bg = bubbles.enter()
        .append('g')
    ...
    bg.append('circle')
            ...
            .attr('stroke', 'black')
            .attr('stroke-width', 1)
            .attr('fill', d=>bubbleColor(d.r));  
    
    //5.2. Add Text under a separate group so that text is not burried below the circles
    const bubbleText = graphText.selectAll('g').data(data);
    bubbleText.enter().filter(d=>d.r>50)
        .append('text')
        .text(d=>d.data.name)
        .attr('x', d=>d.x)
        .attr('y', d=>d.y)
        .attr('fill', 'white')
        .style('font-size', d=>d.r/10)

};

//prep data

//d3.stratify() takes hierarchy data in the format
//{name:"", parent:"", someNum:} and convertes to nested object of the format
//[children:{ [{}, {}, {}] }, data:{}, depth:0, height:4,  ]
const stratify = d3.stratify().id(d=>d.name).parentId(d=>d.parent);

//d3.stratify().sum() creates one more property called "value:" to the stratify data
const rootNode = stratify(worldData).sum(d => d.gdp);

//d3.pack() adds three more properties: x, y and r
const pack = d3.pack()
.size([graphWidth, graphHeight])
.padding(0);

//d3.pack().descendants() convertes the nested stratified objects to flatterned array of objects
